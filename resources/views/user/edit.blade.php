                            ...
                            <form class="p-5 lg:max-w-4xl mx-auto dropzone" action="{{ route('users.update', $user->id) }}"
                                ...
                                <div class="flex gap-6 flex-wrap md:flex-nowrap">
                                    <div class="w-1/2 mx-auto">
                                        ...
                                        <div class="mb-6">
                                            <label for="roles"
                                                class="block  text-sm mb-2 font-medium text-blue-500 dark:text-gray-300">Select
                                                A Role </label>
                                            <select id="roles" name="roles"
                                                class="block text-sm text-center py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                                                <option value="" style="display:none;">Chose A Role Please</option>
                                                @foreach ($roles as $role)
                                                    <option value="{{ $role->name }}"
                                                        @if (in_array($role->id, $data)) selected @endif>
                                                        {{ $role->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('roles')
                                        ...
                                        <div class="mb-6">
                                            <label for="userPassword"
                                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                                Password <small class="text-blue-700">(leave it empty, if you don't want to
                                                    change)</small> </label>
